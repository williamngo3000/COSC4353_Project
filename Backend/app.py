...
DB = {
    "users": {
        ...
        # 👇 Added another test volunteer
        "volunteer2@example.com": {
            "password": "Password2", "role": "volunteer",
            "profile": {
                "full_name": "Alice Johnson", "address1": "789 Oak St",
                "city": "Houston", "state": "TX", "zip_code": "77005",
                "skills": ["Event Setup", "Catering"],
                "preferences": "Prefers weekend activities",
                "availability": ["2024-11-20", "2024-12-10"]
            },
            "history": []
        },
        "admin@example.com": {
    ...
    "events": {
        ...
        2: {
            ...
            "location": "Memorial Park", "required_skills": ["Event Setup", "Catering"],
...
# 🆕 --- Volunteer Self-Matching Route ---
@app.route('/matching/for_volunteer/<string:email>', methods=['GET'])
def match_events_for_volunteer(email):
    user = DB["users"].get(email)
    ...
    if not profile:
        return jsonify({"message": "Volunteer profile is incomplete"}), 400

    volunteer_skills = set(profile.get("skills", []))
    volunteer_availability = set(profile.get("availability", []))

    matched_events = []
    for event_id, event in DB["events"].items():
        required_skills = set(event.get("required_skills", []))
        event_date = event.get("event_date")

        if required_skills.intersection(volunteer_skills) and event_date in volunteer_availability:
    ...
    return jsonify(matched_events), 200
