...
def join_event(event_id):
    ...
    # make sure list exists (covers old events created before this field)
    ...
    DB["users"].setdefault(email, {}).setdefault("history", [])
    if event_id not in DB["users"][email]["history"]:
        DB["users"][email]["history"].append(event_id)

    return jsonify({"message": "Joined", "event": {"id": event_id, **event}}), 200
